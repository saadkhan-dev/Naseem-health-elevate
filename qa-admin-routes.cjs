...
(async () => {
  if (!EMAIL || !PASS) { console.error('MISSING QA_ADMIN_EMAIL/QA_ADMIN_PASS'); process.exit(2); }
  ...
  page.on('response', (res) => { if (res.status() >= 500) failedReqs.push(`HTTP ${res.status()} ${res.url()}`); });

  const settle = async () => { await page.waitForTimeout(2000); };
  ...
  if (!afterLogin.startsWith(BASE + '/admin')) { console.error('LOGIN FAILED'); process.exit(1); }
  ...
  for (const [route, expectPrefix] of ROUTES) {
    ...
    const refreshOk = urlAfterRefresh.startsWith(BASE + expectPrefix) && !urlAfterRefresh.includes('/admin/login');
    ...
    results[results.length - 1].errors = results[results.length - 1].errors.concat(errsAfterRefresh);
  }
  ...
  navResults.push({ op: 'back', url: backUrl, ok: backUrl.startsWith(BASE + '/admin/appointments') });
  navResults.push({ op: 'forward', url: forwardUrl, ok: forwardUrl.startsWith(BASE + '/admin/conditions') });
  ...
  for (const r of results) {
    ...
    if (r.errors.length) r.errors.slice(0, 5).forEach((e) => console.log(`     console-error: ${e.slice(0, 200)}`));
  }
