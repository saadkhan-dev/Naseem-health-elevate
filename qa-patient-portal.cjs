...
(async () => {
  ...
  const url1 = page.url();
  await assert(
    "Logged in → dashboard (URL=/patient)",
    url1 === BASE + "/patient" || url1 === BASE + "/patient/",
  );
  await assert(
    "Dashboard heading visible",
    await page
      .locator('h1:has-text("My Appointments")')
      .isVisible()
      .catch(() => false),
  );
  await assert(
  ...
  await assert(
    "Sidebar shows QA Patient A",
    (await page.locator("div.truncate").first().textContent())?.includes("QA Patient A") ?? false,
  );
  ...
  await assert(
    "Documents heading",
    await page
      .locator('h1:has-text("My Documents")')
      .isVisible()
      .catch(() => false),
  );
  await assert(
  ...
  await settle(5000);
  const uploadRow = await page
    .locator("text=qa-test-doc.txt")
    .isVisible()
    .catch(() => false);
  ...
  await assert(
    "Profile heading",
    await page
      .locator('h1:has-text("My Profile")')
      .isVisible()
      .catch(() => false),
  );
  ...
  await settle(2000);
  await assert(
    "Patient portal shows lock screen after logout",
    await page
      .locator("text=Patient Portal")
      .isVisible()
      .catch(() => false),
