...
if (!url || !serviceKey) { console.error('missing env'); process.exit(2); }

const admin = createClient(url, serviceKey, { auth: { persistSession: false } });

const stamp = '260926';
const A = { email: `qa.patient.a.${stamp}@example.com`, password: 'PatientA#2026$strong', phone: '+920000000101', name: 'QA Patient A' };
const B = { email: `qa.patient.b.${stamp}@example.com`, password: 'PatientB#2026$strong', phone: '+920000000102', name: 'QA Patient B' };
...
(async () => {
  ...
  for (const [key, p] of Object.entries({ A, B })) {
    ...
    if (!found) {
      ...
      if (created.error) { console.error(`createUser ${key} FAILED`, created.error.message); process.exit(1); }
    ...
    if (perr) { console.error(`profile ${key} FAILED`, perr.message); process.exit(1); }
    const prof = await admin.from('profiles').select('id,full_name,phone,role').eq('id', userId).single();
  ...
  fs.writeFileSync(path.join(process.env.TEMP || '/tmp', 'qa-patients.json'), JSON.stringify(out, null, 2));
