...
function AdminDashboard() {
  ...
  const cards = [
    { label: "Total Appointments", value: stats?.totalAppointments ?? 0, Icon: CalendarCheck, color: "text-blue-600", bg: "bg-blue-100" },
    { label: "Pending", value: stats?.pendingAppointments ?? 0, Icon: Clock, color: "text-amber-600", bg: "bg-amber-100" },
    { label: "Today", value: stats?.todayAppointments ?? 0, Icon: Activity, color: "text-green-600", bg: "bg-green-100" },
    { label: "Total Patients", value: stats?.totalPatients ?? 0, Icon: Users, color: "text-purple-600", bg: "bg-purple-100" },
  ];
            ...
            <div className="divide-y">
              {todayAppts.map((a) => (
                <div key={a.id} className="flex items-center justify-between px-5 py-3">
                  <div>
                    <div className="text-sm font-medium text-foreground">{a.patient_name ?? "Unknown"}</div>
                    <div className="text-xs text-muted-foreground">{a.service_name}</div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-sm text-muted-foreground">{formatTimeDisplay(a.time)}</span>
