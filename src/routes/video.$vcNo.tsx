...
function VideoCallPage() {
  ...
  /**
   * The floating consultation chat is the patient-facing chat surface on this
   * page: it appears for signed-in patients who have a live video appointment.
   */
  const canFloatChat = !isDoctor && !!user && !!session && !!join?.appointment;

  /**
   * Open this appointment's persistent consultation chat (the SAME conversation
   * the appointment card links to — never a new one). Patients stay on this
   * page and use the floating chat panel; `joinMeet` just opens Google Meet in
   * a new tab. Only the doctor navigates away to the staff chat page after
   * opening the meeting.
   */
      ...
      <div className="flex min-h-dvh items-center justify-center px-4 py-8">
        <div className="w-full max-w-md rounded-xl border bg-card p-6 shadow-sm">
          ...
          <p className="mt-3 text-center text-xs text-muted-foreground">
            Google Meet will open in a new tab. Use the "Chat with Doctor" button below to open this
            appointment's chat in its own window. Allow camera and microphone access when asked.
          </p>
        </div>
      </div>
      {canFloatChat && (
