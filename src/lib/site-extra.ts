...
import { FALLBACK_DOCTOR_PROFILE } from "@/lib/fallback-content";
...
export async function getDoctorProfile(): Promise<DoctorProfile | null> {
  try {
    ...
    const profile = result.profile as DoctorProfile | null | undefined;
    if (profile) return profile;
  } catch {
    // ignore — fall through to snapshot below
  }
  // Database not connected / profile row missing → use the live site's record.
  return FALLBACK_DOCTOR_PROFILE;
}
